...
# Years to cover – change if you only want a subset
START_YEAR = 1990
END_YEAR = 2025
...
def build_query_with_year(year: int) -> str:
    return f"{PNAS_FILTERS} AND PUB_YEAR:{year}"
...
def process_single_article(article: Dict) -> Dict:
    time.sleep(random.uniform(0.5, 1.0))  # politeness
...
# -----------------------------
# Existing ID loader (to avoid duplicates)
# -----------------------------

def load_existing_ids(meta_path: str) -> set:
    ids = set()
...
def main():
    ...
    try:
        for year in range(START_YEAR, END_YEAR + 1):
            print(f"\n=== Processing PNAS Year {year} ===")
            year_dir = os.path.join(OUTPUT_ROOT, str(year))
            ensure_dir(year_dir)

            query = build_query_with_year(year)
            cursor = "*"
            page_count = 0
            ...
            while True:
                ...
                if not data or "resultList" not in data:
                    print(f"  [Year {year}] No data or end at page {page_count}.")
                    break
                ...
                # Filter out articles already present in metadata_all.jsonl
                new_results = []
                for art in results:
                ...
                if not new_results:
                    print(f"  [Year {year}] Page {page_count}: all articles already present, skipping.")
                else:
                    print(f"  [Year {year}] Page {page_count}: {len(new_results)} NEW PNAS articles to process.")
            ...
            print(f"=== Finished PNAS {year}. New articles added: {year_articles_count} ===")
